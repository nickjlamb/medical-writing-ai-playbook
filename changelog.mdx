...
- **[Choosing Your Model](/principles/choosing-your-model) gains a fourth selection axis: resource cost.** Cost, quality and latency are what you trade against on a single task; at team scale, energy and water move alongside them. The new section covers why over-powered model choice compounds — the tasks already listed as reasoning-model overkill are the ones where the saving is free — why CSRD-driven environmental questions are starting to reach agency RFPs through procurement, and the proportionality point: a frontier model on a high-stakes regulatory document is a good use of resources, the same model generating fifty social captions is not. Sized with the [AI Water and Energy Footprint Calculator](https://www.pharmatools.ai/water-consumption-calculator).
- **[Agentic Workflows](/principles/agentic-workflows)** — new entry under "When an agent is the wrong tool": a single instruction becomes many model calls, so a loop that is not earning its keep multiplies the same overkill rather than causing a different one
- **Homepage rewritten above the fold** — the hero subhead now leads with the accountability tension rather than listing lifecycle stages, and the `description`, `og:description` and `twitter:description` tags are decoupled from it into a single keyword-bearing line, synced with the homepage entry in `product/SEO-METADATA.md` (the two had drifted, so the SEO copy had never reached the page it was written for)

## v2.7 — 16 August 2026

- **[AI Regulation in Pharma](/principles/ai-regulation) updated for Article 50 coming into force.** The EU AI Act's transparency obligations became enforceable on 2 August 2026, and foundation model providers have begun marking their output at source — [Anthropic watermarking Claude's text output globally](https://www.anthropic.com/news/claude-text-watermark), Google's SynthID doing the same for Gemini, and C2PA content credentials on generated image files. The page previously described detectability as a future obligation; it now describes what has actually shipped and what it means for writers:
  - New **Provider-side content marking** section — what text watermarking is, which providers have deployed it, and the limits of detection: there is no public cross-provider detector, Anthropic's detection API is announced but unreleased, and the signal is weakest on short, factually constrained or heavily rewritten text, which describes most medical writing after author review and MLR
  - **The marking obligation sits on the provider, not on you** — a watermark is not a disclosure and changes nothing about what you declare
  - **Article 50(4)'s human editorial review exemption made explicit** — text published under a named person holding editorial responsibility is not, in the Act's own terms, undisclosed synthetic content, which puts the named-reviewer model the playbook argues for throughout on a statutory footing
  - **Implementation timeline** corrected to present tense, with the 2 December 2026 grace period for machine-readable marking of synthetic content added

## v2.6 — 11 August 2026

- **Disclosure guidance updated for JAMA Network's tightened AI policy** ([editorial, 10 August 2026](https://jamanetwork.com/journals/jama/fullarticle/2852772), doi:10.1001/jama.2026.16613). JAMA has moved three categories from "disclose" to "not permitted": AI-generated or AI-formatted references, AI-drafted Opinion articles, Letters to the Editor and online Comments, and AI-created or AI-manipulated clinical images, illustrations, video and audio. The playbook's disclosure pages assumed any use could be disclosed into acceptability; that framing no longer holds, so the change is threaded through five pages rather than confined to the policy table:
  - **[Declaring AI Use](/principles/declaring-ai-use)** — JAMA row rewritten with the full set of prohibitions and the expanded disclosure fields (tool, version, manufacturer, dates of use, portions affected, statement of author responsibility); new framing on the shift from disclosure to prohibition; the "what counts as AI use" table revised, with new rows for Opinion drafting and for permitted adjustment of existing scientific images; two new Common Mistakes entries
  - **[Disclosure Language Template](/templates/disclosure-language)** — reference **verification** (still acceptable) separated from reference **generation** (now prohibited); figure snippet scoped to conceptual visuals, with a new snippet for uniform adjustment of existing scientific images; manufacturer and dates of use added throughout; new section documenting the uses that deliberately have no template because they are barred
  - **[Pre-Submission QC Checklist](/templates/pre-submission-qc-checklist)** — new checks for prohibited uses, AI-free reference compilation, clinical-image provenance, uniform image adjustment and retained originals, plus a sign-off row
  - **[AI in Peer Review](/principles/ai-in-peer-review)** — notes that journals are now responding upstream with policy, not only at the automated screening gate
  - **[Generate Concept Visuals](/workflows/generate-concept-visuals)** — reality check extended to cover journal submissions, not just regulatory and MLR-bound materials

## v2.5 — 26 July 2026

- **[The Description–Discernment Loop](/principles/description-discernment)** added to Principles — applying concepts from the AI Fluency Framework (Dakan & Feller, as taught in Anthropic's AI Fluency courses) to medical writing:
  - **Description** — briefing AI like you'd brief a writer: approved prior work as voice examples, injecting the context AI can't know (client sensitivities, MLR history, audience realities), and describing the approach (source-grounding rules) not just the output — with a confidentiality warning on what may be uploaded
  - **Discernment beyond accuracy** — an explicit "does it sound like us?" voice check, and process-level discernment (recurring corrections belong in the brief, not in another round of edits)
  - **Closing the loop** — a specific-vs-vague revision instruction table, plus two safety rules: pin what must not change, and re-verify anything the model touched
  - **Briefing packs** — a reusable per-client/per-product context document that accumulates what each project teaches you
- **[Voice and Authenticity Check](/prompts/review-prompts#7-voice-and-authenticity-check)** added to Review Prompts — compares a draft against approved reference examples for generic AI register, vocabulary drift, and register mismatch
- **[Final Human Review](/workflows/final-human-review)** checklist extended with a voice check under AI-specific checks

## v2.4 — 16 May 2026

- **Playbook now available as a downloadable PDF** — every page of the playbook (cover, principles, workflows, prompt patterns, tools, templates) bundled into a single A4 PDF with auto-generated table of contents and section dividers. Linked from the homepage hero ("Download as PDF") and the [Start Here](/start) page. Rebuilt automatically on every content update via GitHub Actions and published to GitHub Releases so the download URL always serves the latest version.

## v2.3 — 4 May 2026

- **Five new Principles pages** covering the 2025–26 AI landscape:
  - **[Declaring AI Use](/principles/declaring-ai-use)** — journal, regulator, and sponsor disclosure expectations, with mid-2026 policy snapshot for ICMJE, NEJM, Lancet, JAMA Network, BMJ, Nature, Science/AAAS, and WAME (each linked to the publisher's actual AI policy page)
  - **[AI Regulation in Pharma](/principles/ai-regulation)** — the EU AI Act's risk-tier system, Article 50 transparency obligations, foundation model provider obligations, with brief notes on FDA, EMA, MHRA, WHO, PMDA, and NMPA frameworks
  - **[Choosing Your Model](/principles/choosing-your-model)** — when to reach for a reasoning model vs. a standard LLM vs. a frontier model, with a practical decision matrix and a worked example of mixing model classes across a manuscript workflow
  - **[Agentic Workflows](/principles/agentic-workflows)** — generalising the closed-loop pattern (RefCheckr) to other agent applications, with five agent patterns and the failure modes specific to agentic systems
  - **[AI in Peer Review](/principles/ai-in-peer-review)** — what publishers run on submissions before peer review, where the screens work well vs. where they don't, and a pre-submission QC checklist
...
<Note>
**Version numbering.** Entries below this point were originally numbered v2.0–v2.24. They were renumbered to the v1.x series on 19 August 2026 to clear six duplicate version numbers, created when the current series restarted at v2.3 on 4 May 2026. Only the numbers changed; dates and content are untouched.
</Note>
...
- **[RefCheckr tool page](/tools/refcheckr)** rewritten with a "How the closed loop works" table covering Verify, Fix, and Re-check steps and the underlying stack
- **Closed-loop AI** added to the [glossary](/glossary), anchored to RefCheckr as the working example
...
- **Nano Banana 2** and **Midjourney** added to the [Tool ecosystem](/tools/ecosystem) under a new **AI image generation** section, with an explicit pointer that AI image tools are not substitutes for BioRender or a medical illustrator on scientific figures
- **Decision tree** updated with a new row for concept images, visual abstracts, slide visuals, and social graphics

## v1.22 — 18 April 2026

- **Claude Design** added to the [Tool ecosystem](/tools/ecosystem#claude-design) under a new **Design and brand concepting** section — Anthropic Labs' design surface for pitch decks, leavepieces, marketing collateral, and prototypes, framed for medical writers who are increasingly being asked to generate visual concepts before a designer takes over
