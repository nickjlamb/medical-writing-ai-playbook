...
| **Closed-loop AI** | An AI system that acts, observes the result, and adjusts — repeating that cycle without a human in each step. Contrasts with open-loop use, where the model produces a single output and a human decides what to do next. In medical writing, a closed-loop workflow might draft, run reference checks, compare claims against source documents, and revise until the output passes its own checks. |
