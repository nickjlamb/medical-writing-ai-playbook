...
~30 min with AI support, ~45 min without
The reviewer must be qualified for the content type and risk tier.
...
| Tool | Role |
|---|---|
| [RefCheckr](/tools/refcheckr) | Closed-loop reference accuracy check (verify, rewrite, re-check) as one input to the review |
...
- The content reads in your organisation's (or the client's) voice, not in generic AI register — no residual padding, hedging, or substituted near-synonyms for deliberately chosen terminology (see [the Description–Discernment loop](/principles/description-discernment))
